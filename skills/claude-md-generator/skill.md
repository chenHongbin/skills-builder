...
**关键词**: 项目初始化、新项目设置、CLAUDE.md 创建、配置 Claude Code、项目文档、项目指导文件
...
**重要**: **必须使用 `AskUserQuestion` 工具**，即使用户描述已经很详细。提问可以：
- 确认用户的真实需求和优先级
- 了解用户偏好和工作方式
- 避免做出错误假设
...
2. **必须**使用 `AskUserQuestion` 工具一次性提出 2-4 个问题
...
**绝对不要**:
1. 编造 package.json 中不存在的 scripts
2. 添加通用的开发建议（如"写好注释"）
3. 为已有完善 CLAUDE.md 的项目重复生成
4. 列出详细的文件结构
5. 包含与用户需求无关的章节
